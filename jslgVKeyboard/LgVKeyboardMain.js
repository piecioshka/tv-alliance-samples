...
(function(){
    ...
    window.lgKb = {
        ...
        isInitialized :
            function ()
            {
                return lgKb.bInitialized;
            },
        isShown :
            function ()
            {
                return lgKb.bShowVKeyboard;
            },
        ...
        onShow :                //  user's virtual keyboard show event handler
            function ()
            {
                lgKb.onKeyboardShow();
            },
        onHide :                //  user's virtual keyboard hide event handler
            function ()
            {
                lgKb.onKeyboardHide();
            },
        onKeyboardShow :        //  old user's virtual keyboard show event handler
            function ()
            {
            },
        onKeyboardHide :        //  old user's virtual keyboard hide event handler
            function ()
            {
            },
        onInitialized :
...
//                              lgKb.changeLangJs(lgKb.nextLang);       //  this is called HIDLanguageChange event
        ...
        WindowFocusIn :
            function (event)
            {
               ...
               {
                    var fireShowEvent = !(event.relatedTarget && ((event.relatedTarget.tagName=="TEXTAREA") || ( (event.relatedTarget.tagName=="INPUT") && ( (event.relatedTarget.type=="text") || (event.relatedTarget.type=="password") ) ) ) && (!event.relatedTarget.readOnly) && (!event.relatedTarget.disabled));
                    lgKb.focusIn(event, fireShowEvent);
               }
            },
        WindowFocusOut :
            function (event)
            {
                ...
                {
                    var fireHideEvent = !(event.relatedTarget && ((event.relatedTarget.tagName=="TEXTAREA") || ( (event.relatedTarget.tagName=="INPUT") && ( (event.relatedTarget.type=="text") || (event.relatedTarget.type=="password") ) ) ) && (!event.relatedTarget.readOnly) && (!event.relatedTarget.disabled));
                    lgKb.focusOut(fireHideEvent);
                }
            },
        onClearCaretInfo :
            function ()
...
//              }
        ...
        getVersionOfLgBrowser :
            function ()
            {
                ...
                if(nLgBrowser != -1)
                {
                    return userAgent.slice(nLgBrowser + 11, nLgBrowser + 12);
                }
                else
                {
                    return '999';
...
//                      return;     //  Virtual Keyboard works well on LG Smart TV
        ...
        getAbsOffsetTop :
            function (event)
            {
                // return event.target.getBoundingClientRect().top;
